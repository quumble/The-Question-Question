...
def run(args):
    ...
    if args.arch == "gpt":
        all_rows = [r for r in all_rows if r["word_author"] == "gpt"]
    elif args.arch == "claude":
        all_rows = [r for r in all_rows if r["word_author"] == "claude"]
...
def main():
    p = argparse.ArgumentParser(description="QQ-v1 runner")
